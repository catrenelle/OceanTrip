    ...
    class PassTheTime
    {
		...
		public static async Task Craft()
		{
			if (freeToCraft)
			{
				...
				{
					//Potions
					while (freeToCraft && (DataManager.GetItem(29979).ItemCount() >= 40))
					{
						await IdleLisbeth(29492, 60, "Alchemist", "false"); //Grade 3 Tincture of Strength
					}

					//Food
					while (freeToCraft && DataManager.GetItem(30482).ItemCount() < 500)
					{
					...
					//Materia
					while (freeToCraft && DataManager.GetItem(25194).ItemCount() < 30)
					{
						await IdleLisbeth(25194, 10, "Exchange", "false"); //crafter materia
					}
					while (freeToCraft && DataManager.GetItem(25195).ItemCount() < 30)
					{
						await IdleLisbeth(25195, 10, "Exchange", "false"); //crafter materia
					}
					while (freeToCraft && DataManager.GetItem(25196).ItemCount() < 30)
					{
		...
		public static async Task IdleLisbeth(int itemId, int amount, string type, string quicksynth)
		{
